...
class JMethodDeclaration extends JAST implements JMember {
    ...
    /**
     * Exceptions thrown, stored as Strings.
     */
    protected ArrayList<String> exceptionsStrings;
    ...
    public JAST analyze(Context context) {
        ...
        // Generate an ArrayList of Strings for the exceptions.
        if (exceptions != null) {
            exceptionsStrings = new ArrayList<String>();
            for (TypeName exception : exceptions) {
                exceptionsStrings.add(exception.jvmName());
            }
        }
    ...
    public void codegen(CLEmitter output) {
        output.addMethod(mods, name, descriptor, exceptionsStrings, false);
        if (body != null) {
            body.codegen(output);
        }
        if (returnType == Type.VOID) {
