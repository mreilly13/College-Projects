...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                if (ch == '/') {
                    ...
                    if (ch == EOFCH) {
                        reportScannerError("EOF reached inside comment");
                    }
                } else if (ch == '=') {
                    nextCh();
        ...
        switch (ch) {
            ...
            case '.':
                nextCh();
                if (isDigit(ch)) {
                    return scanNumLiteral(new StringBuffer("."), true);
                } else {
            ...
            case '*':
                ...
                if (ch == '=') {
                    nextCh();
                    return new TokenInfo(TIMES_ASSIGN, line);
                } else {
            ...
            case '8':
            case '9':
                return scanNumLiteral(new StringBuffer(), false);
            default:
    ...
    /**
     * Helper method to scan and validate a numeric literal.
     *
     * @return the next scanned token of the appropriate numeric literal type.
     */
    private TokenInfo scanNumLiteral(StringBuffer sb, boolean hasDot) {
        boolean isDouble = hasDot;
        while (isDigit(ch)) {
            sb.append(ch);
            nextCh();
        }
        if (ch == '.') {
            if (hasDot) {
                reportScannerError("Extra decimal point found");
                return new TokenInfo(DOUBLE_LITERAL, sb.toString(), line);
            } else {
                sb.append(ch);
                hasDot = true;
                isDouble = true;
        ...
        if (ch == 'e' || ch == 'E') {
            ...
            if (ch == '+' || ch == '-') {
                ...
                if (isDigit(ch)) {
                    ...
                    reportScannerError(ch + " found where decimal digit is expected");
        ...
        if ((ch == 'l' || ch == 'L')) {
            if (isDouble) {
                reportScannerError("Literal is of type double, but has long identifier");
                return new TokenInfo(DOUBLE_LITERAL, sb.toString(), line);
            } else {
                sb.append(ch);
                nextCh();
                return new TokenInfo(LONG_LITERAL, sb.toString(), line);
            }
        } else if (ch == 'd' || ch == 'D') {
            sb.append(ch);
            nextCh();
